...
class Process:
    ...
    def create_from_cwl(cls, cwl, workflow_id=None):
        ...
        if '$namespaces' in cwl and isinstance(cwl['$namespaces'], dict):
            for (prefix, uri) in cwl['$namespaces'].items():
                if uri in ['https://schema.org/SoftwareApplication', 'https://schema.org/']:
                    software_namespace_prefix = prefix

        workflows = [item for item in cwl['$graph'] if 'class' in item and item['class'] == 'Workflow']
        if len(workflows) == 0:
            raise Exception("No workflow found")
        if workflow_id is None:
            workflow = workflows[0]
        else:
            workflow = next((wf for wf in workflows if 'id' in wf and wf['id'] == workflow_id), None)
            if workflow is None:
    ...
    def write_zcfg(self, stream):
        ...
        print("  storeSupported = {0}".format('true' if self.store_supported else 'false'), file=stream)
        print("  statusSupported = {0}".format('true' if self.status_supported else 'false'), file=stream)
        ...
        for input in self.inputs:
            ...
            print("      maxOccurs = {0}".format(999 if input.max_occurs == 0 else input.max_occurs), file=stream)
            if input.is_complex:
                pass
            else:
                ...
                if input.possible_values:
                    print("        AllowedValues = {0}".format(','.join(input.possible_values)), file=stream)
                if input.default_value:
                    print("        <Default>", file=stream)
                    print("          value = {0}".format(input.default_value), file=stream)
                    print("        </Default>", file=stream)
                else:
                    print("        <Default/>", file=stream)
                print("      </LiteralData>", file=stream)
        ...
        for output in self.outputs:
            ...
            if output.is_complex:
                ...
                print("        <Default>", file=stream)
                print("          mimeType = {0}".format(output.file_content_type if output.file_content_type else 'text/plain'), file=stream)
                print("        </Default>", file=stream)
                print("      </ComplexData>", file=stream)
            else:
                print("      <LiteralData>", file=stream)
    ...
    def get_ogc_api_json(self):
        ...
        for input in self.inputs:
            ...
            if input.max_occurs != 1:
                ...
                ogc_input_schema['maxItems'] = input.max_occurs if input.max_occurs > 1 else 100
                ogc_input_schema['items'] = { 
                    'type': input.type
                }
            if input.possible_values:
...
class ProcessInput:
    ...
    cwl_input_type_regex = re.compile(r"^(?P<name>[A-Za-z_][A-Za-z0-9_]+?)(?P<array>\[\])?(?P<optional>\?)?$")
    ...
    def set_type_from_cwl(self, input_id, cwl_input_type):
        if isinstance(cwl_input_type, str):
            ...
            if not type_match:
                raise Exception("Incorrect type for input '{0}': '{1}'".format(input_id, cwl_input_type))
            ...
            if type_name in self.__class__.cwl_type_map:
                ...
                raise Exception("Unsupported type for input '{0}': {1}".format(input_id, type_name))

            self.type = type_name
            self.min_occurs = 0 if type_match.group('optional') else 1
            self.max_occurs = 0 if type_match.group('array') else 1   # 0 means unbounded, TODO: what should be the maxOcccurs value if unbounded is not available?

        elif isinstance(cwl_input_type, dict):
            if 'type' not in cwl_input_type:
                raise Exception("Type missing for input '{0}': '{1}'".format(input_id, cwl_input_type))

            type_match = self.__class__.cwl_input_type_regex.match(cwl_input_type['type'])
            ...
            if type_name == 'enum':
                ...
                if 'symbols' not in cwl_input_type:
                    raise Exception("Missing symbols (possible values) for enum input '{0}'".format(input_id))
                elif not isinstance(cwl_input_type['symbols'], list):
                    raise Exception("Symbols (possible values) are not a list for enum input for '{0}'".format(input_id))
                self.possible_values = [ str(s) for s in cwl_input_type['symbols'] ]
            elif type_name == 'array':
                if 'items' not in cwl_input_type:
                    raise Exception("Missing item type for array input '{0}'".format(input_id))
    ...
    def set_type_from_cwl(self, output_id, cwl_output_type):
        if isinstance(cwl_output_type, str):
            ...
            if type_name == 'string':
                ...
                raise Exception("Unsupported type for output '{0}': {1}".format(output_id, type_name))
